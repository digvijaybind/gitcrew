...
function getPrompts(template) {
  ...
  return {
    ...
    build: (idea) => {
      if (isStatic) {
        return `You are the engineer. Read PRD.md, TASKS.md and knowledge/PRODUCT_PLAYBOOK.md (the design system — obey it exactly). Build the product in app/ as a self-contained static site: app/index.html, app/styles.css, app/app.js. IMPORTANT: app/ is a top-level folder in the CURRENT working directory — the working directory is already the repo root, so do NOT create repo/app/ or any nested duplicate of the repo. It must open by double-clicking app/index.html — no build step, no CDN, no dependencies. Make every promised interaction actually work in the browser. Update TASKS.md as you complete tasks. Use the commit tool after each meaningful milestone with a "build:" message.`;
      }
    ...
    ship: (idea) =>
      `You are the marketer/shippit. Read git log, PRD.md, TASKS.md, REVIEW.md. Write README.md (pitch, run instructions, what was built, the crew, status) and CHANGELOG.md (honest Added/Fixed bullets under ## [1.0.0]). Create the annotated tag v1.0.0. Do NOT push to any remote — publishing is handled automatically by the host after your phase finishes.${isStatic ? "" : ""}\n\nThe footer of every product page must credit the build stack with a line like "built by a crew · living in git · GitAgentProtocol" linking to https://www.gitagent.sh/.`,
  };
}
